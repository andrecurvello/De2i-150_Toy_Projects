...
BOOL VIP_Config(PCIE_HANDLE hPCIe, int Width, int Height){
	...
	bPass = PCIE_Write32(hPCIe, DEMO_PCIE_USER_BAR, VIP_FRAME_READER_ADDR+6*4, word); // The number of single-cycle color patterns to read for the frame
	...
	bPass = PCIE_Write32(hPCIe, DEMO_PCIE_USER_BAR, VIP_FRAME_READER_ADDR+13*4, word); // The number of single-cycle color patterns to read for the frame
...
BOOL VGA_Display(PCIE_HANDLE hPCIe, BOOL bInvert, int nActiveFrame){
	...
	unsigned char *pPattern, ExpectedData;
	char szError[256];
